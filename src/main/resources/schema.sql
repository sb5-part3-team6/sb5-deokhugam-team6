...
COMMENT ON COLUMN notifications.type IS 'LIKE, COMMENT, REVIEW';
COMMENT ON COLUMN ranks.type IS '일간(DAILY) 주간(WEEKLY) 월간(MONTHLY) 역대(ALL_TIME)';
COMMENT ON COLUMN ranks.target IS 'BOOK REVIEW USER';